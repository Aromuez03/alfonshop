...
<body>
    ...
    <div class="container">
        <div class="card">
            ...
            <div class="card-body">
                <form action="/Catalogo/ActualizarProducto/${producto.getId()}" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input type="url" class="form-control" id="urlImg" name="urlImg" value="${producto.getUrlImg()}" required>
                    </div>
                    <div class="form-group">
                        <label for="categoria">Categoría:</label>
                        <select class="form-control" id="categoria" name="categoria" required>
                            <% for (categoria categoria : (List<categoria>) request.getAttribute("categorias")) { %>
                                <option value="<%= categoria.getId() %>" <% if (categoria.getId() == producto.getCategoria().getId()) { %> selected <% } %>><%= categoria.getNombre() %></option>
                            <% } %>
                        </select>
                    </div>
                    <button type="submit" class="btn btn-danger">Guardar</button> <a href="/Catalogo" class="btn btn-danger" style="margin-left: 10px;">Volver al catalogo</a>
                </form>
            </div>
        </div>
    </div>
